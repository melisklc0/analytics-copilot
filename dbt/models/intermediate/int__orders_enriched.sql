...
payments_agg as (
    ...
    select distinct on (order_id)
        ...
        installments_by_type                                        as dominant_payment_installments,
        sum(payment_value_by_type) over (partition by order_id)     as total_payment_value,
        count(*)                   over (partition by order_id) > 1 as has_multiple_payment_methods
...
select
    ...
    coalesce(i.gross_product_revenue, 0)   as gross_product_revenue,
    coalesce(i.total_freight_charged, 0)   as total_freight_charged,
    coalesce(i.gross_product_revenue, 0)
        + coalesce(i.total_freight_charged, 0) as gross_order_value,

    -- Payment
    p.dominant_payment_type,
    coalesce(p.dominant_payment_installments, 0)     as dominant_payment_installments,
